...
export function renderAuthPageComponent({ appEl, setUser }) {
    ...
    `
		...
		const inputs = appEl.querySelectorAll('.form-inputs input')
		inputs.forEach(input => {
			input.addEventListener('keydown', event => {
				if (event.key === 'Enter') {
					event.preventDefault() // Предотвращаем отправку формы, если она внутри <form>
					loginButton.click() // Программно нажимаем кнопку
				}
			})
		})
		...
		document.getElementById('toggle-button').addEventListener('click', () => {
			...
			imageUrl = '' // Сбрасываем URL изображения при переключении
